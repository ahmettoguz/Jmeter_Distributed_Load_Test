...
kubectl cp ../k8s_Config/slaveIps.txt -n test "$masterName:/jmeter/apache-jmeter-5.6/bin" --insecure-skip-tls-verify
kubectl cp test.sh -n test "$masterName:/jmeter/apache-jmeter-5.6/bin" --insecure-skip-tls-verify
kubectl cp ../jmx_Config/loadtest.jmx -n test "$masterName:/jmeter/apache-jmeter-5.6/bin" --insecure-skip-tls-verify
...
if ! kubectl exec ${masterName} -n test -- /bin/bash -c 'cd /jmeter/apache-jmeter-5.6/bin && chmod +x test.sh && ./test.sh' --insecure-skip-tls-verify ; then
